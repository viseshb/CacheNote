...
using StickyDesk.Core.Speech;
using StickyDesk.Core.ViewModels;
using StickyDesk_App.Controls;
...
public sealed partial class MainPage : Page
{
    ...
    {
        var svc = App.GetService<StickyDesk.Core.Ai.AiAssistService>();
        ...
        List<StickyDesk.Core.Ai.AiAction> planned = new();
    ...
    // Lightweight on-disk diagnostics for the color-overlay flow (read after a live drag).
    private static void Diag(string m, bool reset = false)
    {
        try
        {
            const string path = @"E:\Todo_List\colordiag.txt";
            if (reset) System.IO.File.WriteAllText(path, m + "\n");
            else System.IO.File.AppendAllText(path, m + "\n");
        }
        catch { }
    }
