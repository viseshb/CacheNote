...
/// built StickyDesk.App.exe, launch it, get its main window, and capture
...
internal static class TestApp
{
    ...
    static TestApp() => Environment.SetEnvironmentVariable("STICKYDESK_NO_SINGLE_INSTANCE", "1");

    public static string FindExe()
    {
        var fromEnv = Environment.GetEnvironmentVariable("STICKYDESK_EXE");
        if (!string.IsNullOrWhiteSpace(fromEnv) && File.Exists(fromEnv))
            return fromEnv;

        var slnDir = FindSolutionDir();
        var appBin = Path.Combine(slnDir, "StickyDesk.App", "bin");
        if (!Directory.Exists(appBin))
            throw new FileNotFoundException($"App bin folder not found at {appBin}. Build StickyDesk.App first.");

        var exe = Directory.GetFiles(appBin, "StickyDesk.App.exe", SearchOption.AllDirectories)
            .OrderByDescending(File.GetLastWriteTimeUtc)
            .FirstOrDefault();

        return exe ?? throw new FileNotFoundException("StickyDesk.App.exe not found. Build StickyDesk.App first.");
    }
    ...
    private static string FindSolutionDir()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "StickyDesk.sln")))
            dir = dir.Parent;

        return dir?.FullName ?? throw new DirectoryNotFoundException("Could not locate StickyDesk.sln above the test directory.");
    }
}
